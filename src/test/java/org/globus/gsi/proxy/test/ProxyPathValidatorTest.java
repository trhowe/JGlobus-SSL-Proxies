...
public class ProxyPathValidatorTest extends TestCase {
    ...
    private Log logger = LogFactory.getLog(ProxyPathValidatorTest.class);
    ...
    private void validateChain(X509Certificate[] chain, X509Certificate expectedIdentity, boolean expectedLimited)
        throws Exception {
    ...
    public void testValidatePathProxyPathConstraint() throws Exception {
        ...
        chain = new X509Certificate[] { goodCertsArr[10], goodCertsArr[9], goodCertsArr[12], goodCertsArr[1],
                goodCertsArr[0] };
    ...
    public void testValidateChain() throws Exception {
        ...
        v.reset();
        try {
            v.validate(chain, new X509Certificate[] { goodCertsArr[0] }, null, policies);
        } catch (ProxyPathValidatorException e) {
            logger.debug(e);
    ...
    public void testNoBasicConstraintsExtension() throws Exception {
        ...
        TrustedCertificates trustedCert = new TrustedCertificates(new X509Certificate[] { goodCertsArr[1] },
            new SigningPolicy[] { new SigningPolicy("foo", null) });
    ...
    public void testCrlsChecks() throws Exception {
        ...
        try {
            validator.validate(chain, trustedCerts.getCertificates(), certRevLists, trustedCerts.getSigningPolicies());
        } catch (ProxyPathValidatorException e) {
            logger.debug(e);
            fail("Unexpected exception: " + e.getMessage());
        }
    ...
    public void testSigningPolicy() throws Exception {
        ...
        SigningPolicy policy = SigningPolicyParser.getPolicy(new StringReader(signingPolicy),
        ...
        policy = SigningPolicyParser.getPolicy(new StringReader(signingPolicyViolation),
            "/C=US/O=Globus/CN=Globus Certification Authority");
        ...
        policy = SigningPolicyParser.getPolicy(new StringReader(signingPolicySansPolicy),
            "/C=US/O=Globus/CN=Globus Certification Authority");
        tc = new TrustedCertificates(new X509Certificate[] { goodCertsArr[0] }, new SigningPolicy[] { policy });
        ...
        try {
            ...
            logger.debug(exp);
            expOccured = true;
            assertEquals(ProxyPathValidatorException.NO_SIGNING_POLICY, exp.getErrorCode());
        }
        assertTrue(expOccured);
    ...
    class TestProxyPathValidator extends ProxyPathValidator {
        ...
        // validation pieces
        protected void checkSigningPolicy(X509Certificate certificate, TrustedCertificates trustedCerts,
            Boolean enforcePolicy) throws ProxyPathValidatorException {
            if (policyChk) {
                super.checkSigningPolicy(certificate, trustedCerts, enforcePolicy);
            }
        }
    }
