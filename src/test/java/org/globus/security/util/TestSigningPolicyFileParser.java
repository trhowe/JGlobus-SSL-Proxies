...
public class TestSigningPolicyFileParser {
    ...
    public void setup() throws Exception {
        ...
        this.successFile = new FileSetupUtil("certificateUtilTest/samplePolicy.signing_policy");

        this.singleAllowedDn = new FileSetupUtil("certificateUtilTest/5aba75cb.signing_policy");

        this.tabTestFiles = new FileSetupUtil[3];
        this.tabTestFiles[0] = new FileSetupUtil("certificateUtilTest/afe55e66.signing_policy");
        this.tabTestFiles[1] = new FileSetupUtil("certificateUtilTest/cf4ba8c8.signing_policy");
        this.tabTestFiles[2] = new FileSetupUtil("certificateUtilTest/49f18420.signing_policy");
    }
    ...
    public void testPatternMatching() throws Exception {
        ...
        String patternR = (SigningPolicyFileParser.getPattern(patternStr)).pattern();
        ...
        assertTrue(("CN=" + SigningPolicyFileParser.WILDCARD_PATTERN + "def" + SigningPolicyFileParser.SINGLE_PATTERN
            + "gh" + SigningPolicyFileParser.SINGLE_PATTERN).equals(pattern1R));
        ...
        assertTrue(("/CN=abc" + SigningPolicyFileParser.WILDCARD_PATTERN + "def"
            + SigningPolicyFileParser.WILDCARD_PATTERN + "gh").equals(pattern2R));
        ...
        X500Principal subject22 = new X500Principal("CN=123abc12def34defdef56gh");
        assertFalse(policy.isValidSubject(subject22));

        X500Principal subject23 = new X500Principal("CN=abc12def34defdef56gh123");
        ...
        X500Principal subject32 = new X500Principal("CN=123abc12def34defdef56gh555");
        ...
        X500Principal subject41 = new X500Principal("CN=Globus Certification Authority, O=Globus, C=US");
        ...
        X500Principal subject51 = new X500Principal("CN=Globus Certification Authority, O=Globus, O=US");
        assertFalse(policy.isValidSubject(subject51));
        X500Principal subject52 = new X500Principal("CN=test space,CN=a12b,CN=foo-1,O=Globus,C=US,C=SOME");
        assertTrue(policy.isValidSubject(subject52));
        X500Principal subject53 = new X500Principal("CN=,CN=foo-k,O=Globus,C=US");
        assertTrue(policy.isValidSubject(subject53));
        X500Principal subject54 = new X500Principal("CN= , CN=foo-1, O=Globus, C=US");
        assertTrue(policy.isValidSubject(subject54));

        X500Principal subject55 = new X500Principal("C=US,O=Globus,CN=foo-123,CN=");
        ...
        X500Principal subject61 = new X500Principal("CN=user-12,CN=foo,O=Globus,C=US");
        assertFalse(policy.isValidSubject(subject61));
        X500Principal subject62 = new X500Principal("CN=user-12,CN=foo,O=Global,C=US");
        assertTrue(policy.isValidSubject(subject62));
        X500Principal subject63 = new X500Principal("CN=user-12,CN=bar 1,CN=foo ,O=global,C=US");
        ...
        X500Principal subject71 = new X500Principal("CN=user-12, CN=bar 1, CN=foo , O=Globus,C=US");
    ...
    public void testFileSuccess() throws Exception {
        ...
        Map<X500Principal, SigningPolicy> map = parser.parse(this.successFile.getAbsoluteFilename());

        assertTrue(map != null);

        SigningPolicy policy = map.get(new X500Principal("CN=Globus Certification Authority,O=Globus,C=US"));
        ...
        assertTrue(patterns.contains(p1.pattern()));
        p1 = SigningPolicyFileParser.getPattern("/C=us/O=National Computational Science Alliance/*");
        ...
        policy = map
            .get(new X500Principal("OU=Certification Authority,O=National Computational Science Alliance,C=US"));
    ...
    public void testFilesWithTab() throws Exception {
        ...
        Map<X500Principal, SigningPolicy> map = parser.parse(this.tabTestFiles[0].getAbsoluteFilename());

        SigningPolicy policy = map.get(new X500Principal("CN=CyGridCA,O=HPCL,O=CyGrid,C=CY"));
        assertTrue(policy != null);
        List<Pattern> allowedDN = policy.getAllowedDNs();
        assertTrue(allowedDN != null);
        assertTrue(allowedDN.size() == 1);
        ...
        // given the getPattern method is already tested, assuming it
        // works here.
        Pattern p1 = SigningPolicyFileParser.getPattern("/C=FR/O=CNRS/CN=CNRS-Projets");
        assertTrue(patterns.contains(p1.pattern()));
        ...
        policy = map.get(new X500Principal("CN=INFN Certification Authority,O=INFN,C=IT"));
        assertTrue(policy != null);
        allowedDN = policy.getAllowedDNs();
        assertTrue(allowedDN != null);
        assertTrue(allowedDN.size() == 2);
    ...
    public void testParsingFailure() throws Exception {
        ...
        // not x509
        String error1 = "access_id_CA      notX509         '/C=US/O=Globus/CN=Globus "
        ...
        // not globus
        error1 = "access_id_CA      X509         '/C=US/O=Globus/CN=Globus "
        ...
        // allowed DN
        error1 = "access_id_CA      X509         '/C=US/O=Globus/CN=Globus Certification "
            + "Authority'\n pos_rights        globus        CA:sign\n cond_subjects"
        ...
        error1 = "access_id_CA X509         '/C=US/O=Globus/CN=Globus Certification "
            ...
            + "\"/C=US/O=Globus/*\"' \n cond_subjects     globus       " + "'\"/C=us/O=Globus/*\"'";
        map = parser.parse(new StringReader(error1));
        policy = map.get(new X500Principal("CN=Globus Certification Authority,O=Globus,C=US"));
        assertTrue(policy != null);
        allowedDN = policy.getAllowedDNs();
        assertTrue(allowedDN != null);
        assertTrue(allowedDN.size() == 2);
