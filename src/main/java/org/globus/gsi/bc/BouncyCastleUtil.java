...
public class BouncyCastleUtil {
    ...
    /**
     * Returns certificate type of the given certificate. Please see
     * {@link #getCertificateType(TBSCertificateStructure, TrustedCertificates) getCertificateType} for
     * details for determining the certificate type.
     * 
     * @param cert
     *            the certificate to get the type of.
     * @param trustedCerts
     *            the trusted certificates to double check the {@link GSIConstants#EEC GSIConstants.EEC}
     *            certificate against.
     * @return the certificate type as determined by
     *         {@link #getCertificateType(TBSCertificateStructure, TrustedCertificates) getCertificateType}.
     * @exception CertificateException
     *                if something goes wrong.
     */
    public static int getCertificateType(X509Certificate cert, TrustedCertificates trustedCerts)
        throws CertificateException {
        try {
            return getCertificateType(getTBSCertificateStructure(cert), trustedCerts);
        } catch (IOException e) {
    ...
    /**
     * Returns certificate type of the given certificate. Please see
     * {@link #getCertificateType(TBSCertificateStructure) getCertificateType} for details for determining the
     * certificate type.
     * 
     * @param cert
     *            the certificate to get the type of.
    ...
    /**
     * Returns certificate type of the given certificate. This function calls
     * {@link #getCertificateType(TBSCertificateStructure) getCertificateType} to get the certificate type. In
     * case the certificate type was initially determined as {@link GSIConstants#EEC GSIConstants.EEC} it is
     * checked against the trusted certificate list to see if it really is a CA certificate. If the
     * certificate is present in the trusted certificate list the certificate type is changed to
     * {@link GSIConstants#CA GSIConstants.CA}. Otherwise, it is left as it is (This is useful in cases where
     * a valid CA certificate does not have a BasicConstraints extension)
     * 
     * @param crt
     *            the certificate to get the type of.
     * @param trustedCerts
     *            the trusted certificates to double check the {@link GSIConstants#EEC GSIConstants.EEC}
     *            certificate against. If null, a default set of trusted certificate will be loaded from a
     *            standard location.
     * @return the certificate type. The certificate type is determined by rules described above.
     * @exception IOException
     *                if something goes wrong.
     * @exception CertificateException
     *                for proxy certificates, if the issuer DN of the certificate does not match the subject
     *                DN of the certificate without the last <I>CN</I> component. Also, for GSI-3 proxies when
     *                the <code>ProxyCertInfo</code> extension is not marked as critical.
     */
    public static int getCertificateType(TBSCertificateStructure crt, TrustedCertificates trustedCerts)
     ...
     * The certificate type is {@link GSIConstants#CA GSIConstants.CA} <B>only</B> if the certificate contains
     * a BasicConstraints extension and it is marked as CA.<BR>
     * A certificate is a GSI-2 proxy when the subject DN of the certificate ends with <I>"CN=proxy"</I>
     * (certificate type {@link GSIConstants#GSI_2_PROXY GSIConstants.GSI_2_PROXY}) or
     * <I>"CN=limited proxy"</I> (certificate type {@link GSIConstants#GSI_2_LIMITED_PROXY
     * GSIConstants.LIMITED_PROXY}) component and the issuer DN of the certificate matches the subject DN
     * without the last proxy <I>CN</I> component.<BR>
     * A certificate is a GSI-3 proxy when the subject DN of the certificate ends with a <I>CN</I> component,
     * the issuer DN of the certificate matches the subject DN without the last <I>CN</I> component and the
     * certificate contains {@link ProxyCertInfo ProxyCertInfo} critical extension. The certificate type is
     * {@link GSIConstants#GSI_3_IMPERSONATION_PROXY GSIConstants.GSI_3_IMPERSONATION_PROXY} if the policy
     * language of the {@link ProxyCertInfo ProxyCertInfo} extension is set to
     * {@link ProxyPolicy#IMPERSONATION ProxyPolicy.IMPERSONATION} OID. The certificate type is
     * {@link GSIConstants#GSI_3_LIMITED_PROXY GSIConstants.GSI_3_LIMITED_PROXY} if the policy language of the
     * {@link ProxyCertInfo ProxyCertInfo} extension is set to {@link ProxyPolicy#LIMITED ProxyPolicy.LIMITED}
     * OID. The certificate type is {@link GSIConstants#GSI_3_INDEPENDENT_PROXY
     * GSIConstants.GSI_3_INDEPENDENT_PROXY} if the policy language of the {@link ProxyCertInfo ProxyCertInfo}
     * extension is set to {@link ProxyPolicy#INDEPENDENT ProxyPolicy.INDEPENDENT} OID. The certificate type
     * is {@link GSIConstants#GSI_3_RESTRICTED_PROXY GSIConstants.GSI_3_RESTRICTED_PROXY} if the policy
     * language of the {@link ProxyCertInfo ProxyCertInfo} extension is set to any other OID then the above.<BR>
     * The certificate type is {@link GSIConstants#EEC GSIConstants.EEC} if the certificate is not a CA
     * certificate or a GSI-2 or GSI-3 proxy.
     * 
     * @param crt
     ...
     * @exception IOException
     *                if failed to extract the KeyUsage extension value.
     */
    ...
    /**
     * Creates a <code>ProxyCertInfo</code> object from given extension.
     * 
     * @param ext
     *            the extension.
     ...
     * 
     * @param cert
     *            the certificate to get the subject of. The certificate must be of
     *            <code>X509CertificateObject</code> type.
    ...
    /**
     * Finds the identity certificate in the given chain and returns the subject DN of that certificate in the
     * Globus format.
     * 
     * @param chain
     *            the certificate chain to find the identity certificate in. The certificates must be of
     *            <code>X509CertificateObject</code> type.
     * @return the subject DN of the identity certificate in the Globus format.
     * @exception CertificateException
     *                if something goes wrong.
     */
    ...
    /**
     * Finds the identity certificate in the given chain. The identity certificate is the first certificate in
     * the chain that is not an impersonation proxy (full or limited)
     * 
     * @param chain
     *            the certificate chain to find the identity certificate in.
