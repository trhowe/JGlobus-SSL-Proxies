...
public class GlobusGSSContextTest extends TestCase {
    ...
    protected void setUp() throws Exception {
        ...
        System.setProperty("org.globus.gsi.gssapi.provider", "org.globus.gsi.gssapi.GlobusGSSManagerImpl");
        ...
        clientContext = manager.createContext(gssName, GSSConstants.MECH_OID, null, GSSContext.DEFAULT_LIFETIME);
    ...
    private void establishContext() throws Exception {
        ...
        while (!clientContext.isEstablished()) {
            outToken = clientContext.initSecContext(inToken, 0, inToken.length);

            if (outToken == null || outToken.length == 0) {
                fail("bad token");
            }

            inToken = serverContext.acceptSecContext(outToken, 0, outToken.length);
    ...
    public void testInquireByOid() throws Exception {
        ...
        sc.setOption(GSSConstants.REQUIRE_CLIENT_AUTH, Boolean.FALSE);
    ...
    public void testDelegation() throws Exception {
        ...
        assertTrue((proxy.getProxyType() == GSIConstants.GSI_2_PROXY)
            || (proxy.getProxyType() == GSIConstants.GSI_3_IMPERSONATION_PROXY)
            || (proxy.getProxyType() == GSIConstants.GSI_4_IMPERSONATION_PROXY));
        ...
        clientContext = manager.createContext(gssCred.getName(), GSSConstants.MECH_OID, cred,
        ...
        assertTrue((proxy.getProxyType() == GSIConstants.GSI_2_LIMITED_PROXY)
            || (proxy.getProxyType() == GSIConstants.GSI_3_LIMITED_PROXY)
            || (proxy.getProxyType() == GSIConstants.GSI_4_LIMITED_PROXY));
        logger.debug(proxy);
    }
    ...
    public void testNewDelegation() throws Exception {
        ...
        cl.setOption(GSSConstants.GSS_MODE, GSIConstants.MODE_SSL);
        sr.setOption(GSSConstants.GSS_MODE, GSIConstants.MODE_SSL);
    ...
    public void testContextExpiration() throws Exception {
        ...
        ExtendedGSSContext ctx = (ExtendedGSSContext) clientContext;
        ctx.setOption(GSSConstants.CHECK_CONTEXT_EXPIRATION, Boolean.TRUE);
        ...
        try {
            clientContext.verifyMIC(MSG, 0, MSG.length, MSG, 0, MSG.length, null);
    ...
    public void testLimitedProxyChecking() throws Exception {
        ...
        // create client ctx using default creds
        clientContext = manager.createContext(null, GSSConstants.MECH_OID, cred, GSSContext.DEFAULT_LIFETIME);

        ExtendedGSSContext sr = (ExtendedGSSContext) serverContext;
        sr.setOption(GSSConstants.REJECT_LIMITED_PROXY, Boolean.TRUE);
        ...
        // create client ctx using default creds
        clientContext = manager.createContext(null, GSSConstants.MECH_OID, null, GSSContext.DEFAULT_LIFETIME);

        ExtendedGSSContext cl = (ExtendedGSSContext) clientContext;
        cl.setOption(GSSConstants.REJECT_LIMITED_PROXY, Boolean.TRUE);
    ...
    public void testAnonymousClient1() throws Exception {
        ...
        ExtendedGSSContext sr = (ExtendedGSSContext) serverContext;
        sr.setOption(GSSConstants.REQUIRE_CLIENT_AUTH, Boolean.FALSE);
    ...
    public void testAnonymousClient2() throws Exception {
        ...
        GSSCredential anonCred = manager.createCredential(anonName, GSSCredential.INDEFINITE_LIFETIME, (Oid) null,
            GSSCredential.INITIATE_AND_ACCEPT);
        assertTrue(anonCred.getName().isAnonymous());

        // client ctx initalized with anon cred
        clientContext = manager.createContext(null, GSSConstants.MECH_OID, anonCred, GSSContext.DEFAULT_LIFETIME);
    ...
    public void testBadUsage1() throws Exception {
        ...
        GSSCredential cred = manager.createCredential(null, GSSCredential.DEFAULT_LIFETIME, (Oid) null,
    ...
    private void runMicTests() throws Exception {
        ...
        for (int i = 0; i < msgSize.length; i++) {
            ...
            serverContext.verifyMIC(wToken, 0, wToken.length, msg, 0, msg.length, null);
        }
    }
    ...
    private void runBadMicTest(int off, int expectedError) throws Exception {
        ...
        try {
            serverContext.verifyMIC(mic, 0, mic.length, MSG, 0, MSG.length, null);
    ...
    public void testOldToken() throws Exception {
        ...
        serverContext.verifyMIC(mic1, 0, mic1.length, MSG, 0, MSG.length, null);

        serverContext.verifyMIC(mic2, 0, mic2.length, MSG, 0, MSG.length, null);
