...
/**
 * Extensions to the PureTLS's <code>SSLContext</code> class to allow setting the context credentials and
 * trusted certificates as objects instead of reading them from files. Also allows the use of standard Java
 * security objects for credential and trusted certificates instead of proprietary PureTLS/Cryptix format.
 */
public class PureTLSContext extends SSLContext {
    ...
    private static PrivateKey convertPrivateKey(PrivateKey key) {
        if (key instanceof RSAPrivateCrtKey) {
            ...
            RawRSAPrivateKey ck = new RawRSAPrivateKey(rsaKey.getPrivateExponent(), rsaKey.getPrimeP(), rsaKey
                .getPrimeQ(), rsaKey.getCrtCoefficient());
